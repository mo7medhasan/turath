...
export function PolicyNav({ links, isRtl }: PolicyNavProps) {
  return (
    <nav
      ...
      className="hidden md:block border-b border-(--primary-dark)/20 bg-(--primary-dark)/30"
    >
      <ul className="mx-auto max-w-7xl px-4 flex items-center justify-end gap-1 h-9">
        {links.map((link) => (
          <li key={link.href}>
